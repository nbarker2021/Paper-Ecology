...
The Higgs mass is anchored by construction: `m_H = 5 · κ · SCALE = 125.25` GeV, where `κ = ln(φ)/16` is the golden-ratio constant and `SCALE` is the energy scale factor. The VOA weight assignment is hypothesized for Standard Model particles: W = 3.5, Z = 4, top = 7, etc. The assignment is consistent with empirical masses to within order of magnitude, but the exact numeric calibration is pending.
...
**Keywords:** mass-residue carrier, Arf invariant, VOA weight, correction residue, `2×2×2` vs `3×3`, Higgs doublet, mass-gravity drive, Higgs mass anchor, VOA weight assignment.
...
**Higgs mass anchor.** The equation `m_H = 5 · κ · SCALE = 125.25` GeV, where `κ = ln(φ)/16` and `SCALE` is the energy scale factor. This is the only external input in the series.

**VOA weight assignment.** The hypothesized assignment of VOA conformal weights to SM particles. The structural derivation is complete; numeric calibration is pending.
...
**Proof.** From Paper 2 (Correction Surface Decomposition), `Rule30 = Rule90 xor correction = (L xor R) xor (C and not R)`. Expanding:
```text
...
**Proof.** Exhausting all eight states: `C AND NOT R = 1` iff `C = 1` and `R = 0`. The states satisfying this are exactly `(0,1,0)` and `(1,1,0)`. These are the two shell-2 states in the chiral doublet (Paper 2). ∎
...
### Theorem 15.10 — Higgs Mass Anchor

The Higgs mass is anchored by construction: `m_H = 5 · κ · SCALE = 125.25` GeV, where `κ = ln(φ)/16` and `SCALE` is the energy scale factor. This is the only external input in the series.

**Proof.** Direct from the equation. The anchor is the equation; the equation defines the Higgs mass. The anchor sets the energy scale for all other masses. All other constructions (LCR carrier, `J_3(O)` atlas, Rule 30 ANF, F4 action, magic square) are internal. ∎

### Theorem 15.11 — VOA Weight Assignment

The Standard Model particle masses are assigned VOA weights. The assignment is consistent with the empirical masses to within order of magnitude. The structural derivation is complete; numeric calibration is pending.

**Proof.** Direct from the `calibrate_units.py` module. The structural derivation gives the VOA weight assignment; the numeric calibration requires external data. The honest status is `structural_derivation_complete_numeric_calibration_pending`. ∎

---

## 4. VOA Weight Assignment and Particle Mass Table

The VOA weight assignment for the Standard Model is:

| Particle | Symbol | VOA Weight | Predicted Mass (GeV) | Empirical Mass (GeV) | Relative Error | Role |
|----------|--------|-----------|---------------------|---------------------|----------------|-------|
| Photon | `γ` | 0.0 | 0.0 | 0.0 | — | gauge boson, massless |
...
The Higgs mass is the anchor (0.0% error by construction). The W and Z bosons have ~9–10% relative error, consistent with order-of-magnitude structural prediction. The top quark has 1.5% error. The light quarks (up, down, strange) have larger relative errors but small absolute errors. The honest status is: structural derivation complete, numeric calibration pending.

---

## 5. Verifiers and Receipts

### 5.1 Mass-Residue Carrier
...
### 5.2 Mass-Residue Literalization
...
### 5.3 Eight States / One Dual Reading
...
9. **15.10:** The Higgs anchor is the only external equation; all other masses are derived from it.
...
- **Paper 16** may turn the mass-residue carrier into a windowed edge reading for continuum analysis.
- **Papers 17+** may attempt to close the McKay-Thompson correction parity and the numerical mass calibration.
- **Papers 33, 49–56** (Higgs/Yukawa/Vacuum Band). The mass-residue carrier is the substrate for the Higgs frame mapping (Paper 33), the mass and Yukawa sector (Papers 49–52), and the Higgs and vacuum sector (Papers 53–56).
...
10. J. C. Baez, "The octonions," *Bull. Amer. Math. Soc.* 39 (2002), 145–205. Cayley-Dickson and exceptional structure.
...
- The Rule 30 F2 split, Arf invariant, and VOA weight partition are verified by exhaustive computation. (D — `mass_residue_carrier_receipt.json`)
- The eight-state / dual-reading theorem and the forced-printout theorem are verified by finite checks. (D — `eight_states_one_dual_reading_receipt.json`, `ninth_is_forced_printout_receipt.json`)
- The `2×2×2` vs `3×3` framing resolution is verified by representation theory checks. (D — `mass_framing_2x2x2_vs_3x3_receipt.json`)
- The mass-gravity drive is verified by finite directed-descent checks. (D — `mass_gravity_drive_receipt.json`)
- The Higgs frame mapping structural counting is verified. (D — `higgs_frame_mapping_receipt.json`)

### Interpretation (I)

- The mass = VOA conformal weight interpretation is the author's structural reading. (I — the underlying finite checks are (D).)
- The Higgs doublet `3+1` mapping is a structural analogy to the Standard Model Higgs mechanism. (I — no physical mass prediction is claimed.)
- The VOA weight assignment for Standard Model particles is a hypothesized structural correspondence, not a derived physical prediction. (I — the mass table shows order-of-magnitude consistency, not exact derivation.)
- The application of the mass-residue carrier to later Higgs/Yukawa papers (Papers 33, 49–56) is the author's structural reading of the broader series.

### Fabrication (X)

- None in this paper. The finite substrate claims are (D) verified. The physical calibration claims are honestly marked as (X) open obligations.

---

## 12. Conclusion

Paper 15 closes the substrate mass-residue carrier. It proves the `F2` obstruction split, the Arf gluing rule, the VOA weight partition, the correction-residue identification, the eight-state / dual-reading theorem, the forced-printout theorem, the `2×2×2` vs `3×3` framing resolution, and the mass-gravity drive. It anchors the Higgs mass at 125.25 GeV and records the VOA weight assignment for all Standard Model particles as a structural hypothesis with order-of-magnitude consistency. This is the CQECMPLX Higgs-adjacent mass-residue physics map. The remaining work is calibration to measured Higgs/QFT quantities — particle masses, electroweak scale, and Yukawa couplings — not the internal carrier proof.
