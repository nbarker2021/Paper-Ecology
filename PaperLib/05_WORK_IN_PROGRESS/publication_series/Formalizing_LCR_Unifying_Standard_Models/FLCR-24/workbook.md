# FLCR-24 Workbook - Energetic Traversal Maps
...
- The `formal.md` file for `FLCR-24`.
...
| Step | Action |
|------|--------|
| Step 1 | Reconstruct the local object: energetic traversal map. |
| Step 2 | Write the receipt row that would demonstrate: internal traversal ledgers can order and compare path costs under declared scalar rules. |
| Step 3 | Route the residue without promoting it: joule conversion, thermodynamic identity, and physical energy claims require calibration. |
...
Analog invariance test: a courier route: the package label can change at checkpoints while the payload remains auditable.
...
| additional routed sources | 15 more entries remain in `SOURCE_PLACEMENT.md` |
...
| Continuum Sampling Bridge | Fill sample rule, target space, norm, error bound, units, and failure/counterexample row. | Loose continuum, GR, plasma, or energy language remains presentation-level or calibration-bound. |
...
|---|---|---|
| Figures `FLCR-24-F1`, `FLCR-24-F2`, `FLCR-24-F3` | planned | Convert text schematics into final diagrams or leave as formal schematic figures. |
| Tables `FLCR-24-T1`-`FLCR-24-T4` | planned | Ensure all claim/evidence/source-card/workbook rows are complete. |
| Glossary | drafted | Replace generic term meanings with paper-local definitions. |
| Appendix FLCR-24-A | drafted | Attach receipt paths, verifier paths, source cards, and remaining external requirements. |
