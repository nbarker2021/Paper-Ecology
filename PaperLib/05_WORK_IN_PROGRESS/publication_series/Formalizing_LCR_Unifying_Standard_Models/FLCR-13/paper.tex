...
\usepackage{array}
\begin{document}
\section*{FLCR-13 - Cellular Automata Prediction Surfaces}
...
\noindent{}-- Defines CA prediction surface as a first-class FLCR object.\\
\noindent{}-- States the local result: finite and bounded CA readouts can be receipt-closed without claiming unbounded Rule 30 future-bit extraction.\\
\noindent{}-- Routes downstream use through claim lanes rather than inherited prose: finite surfaces feed admission, window, and falsifier papers as testable local machines.\\
\noindent{}-- Preserves the residue boundary: unbounded nonperiodicity, cold nth-bit extraction, and full prediction remain separate mathematical obligations.\\
...
\noindent{}-- `12\_CA\_Prediction\_Surface.md`\\
\noindent{}-- `virtuous/12\_CA\_Prediction\_Surface\_VIRTUOUS.md`\\
\noindent{}-- `supplements/12\_INTERNAL\_REASONING\_SUPPLEMENT.md`\\
...
\noindent{}-- **Prediction surface.** A bounded state/readout domain on which prediction is actually evaluated.\\
\noindent{}-- **Silent boundary.** A boundary condition whose behavior is fixed before the CA readout runs.\\
...
\noindent\texttt{|-------|------|-----------|}\\
\noindent\texttt{| Theorem 13.1 | `receipt\_bound\_internal\_result` | finite and bounded CA readouts can be receipt-closed without claiming unbounded Rule 30 future-bit extraction |}\\
\noindent\texttt{| Proposition 13.2 | `normal\_form\_result` | CA prediction surface can be imported by later papers only through its declared source, receipt, and lane. |}\\
\noindent\texttt{| Protocol 13.3 | `falsifier\_or\_open\_obligation` | unbounded nonperiodicity, cold nth-bit extraction, and full prediction remain separate mathematical obligations |}\\
...
\noindent\texttt{|---|---|---|---|---|---|}\\
\noindent\texttt{| Theorem 13.1 | finite/executable result | finite and bounded CA readouts can be receipt-closed without claiming unbounded Rule 30 future-bit extraction | receipt, validator, executable pass, generated artifact, or fini}\\
\noindent\texttt{| Proposition 13.2 | formal construction | CA prediction surface can be imported by later papers only through its declared source, receipt, and lane | definitions, normal-form construction, conversion rule, and downstrea}\\
\noindent\texttt{| Protocol 13.3 | obligation/falsifier | unbounded nonperiodicity, cold nth-bit extraction, and full prediction remain separate mathematical obligations | explicit missing derivation, adapter, receipt, dataset, comparato}\\
...
\noindent\texttt{|---|---|---|---|}\\
\noindent\texttt{| Formal-system result | `FLCR-13` defines or uses **Cellular Automata Prediction Surfaces** as a typed FLCR object with declared inputs, operations, outputs, and residue. | Definitions, formal claims, construction steps}\\
...
\noindent\texttt{|---|---|---|---|}\\
\noindent\texttt{| paper lift enumeration | `receipt\_bound\_internal\_result` | 8/8 LCR states succeeded; error walls=0; avg TarPit mass=0.510851. | This closes the paper-local finite lift readout, not every stronger physical interpretatio}\\
...
\noindent\texttt{| CA prediction surface | `standard\_theorem\_citation\_bound\_result` | Finite cellular-automata prediction surfaces are closed where rule and state domain are declared. | Empirical prediction requires dataset and comparato}\\
...
\noindent{}-- unbounded nonperiodicity, cold nth-bit extraction, and full prediction remain separate mathematical obligations\\
...
Assigned original ribbon role(s): `12`/residual\_action.

\noindent\texttt{| Original slot | Family | Lift | Role | Proof form |}\\
\noindent\texttt{|---|---|---:|---|---|}\\
\noindent\texttt{| `12` | residual\_action | 1 | order-2 slot-2: mark the correction, residue, vacancy, or mismatch surface | residual accounting and bounded/unbounded split |}\\
...
\noindent\texttt{|---|---|---:|---|}\\
\noindent\texttt{| `12` | residual\_action | 1 | residual accounting and bounded/unbounded split |}\\
...
\noindent\texttt{|---|---|}\\
\noindent\texttt{| `12\_CA\_Prediction\_Surface.md` | primary assigned source for the paper's formal spine |}\\
\noindent\texttt{| `supplements/12\_INTERNAL\_REASONING\_SUPPLEMENT.md` | paper-local reasoning supplement contributing to definitions, proof sketch, and limitations |}\\
\noindent\texttt{| `virtuous\textbackslash{}12\_CA\_Prediction\_Surface\_VIRTUOUS.md` | prior enriched paper body; should be mined for mature claims and proof ordering |}\\
...
\noindent\texttt{| 12 | 84 | bounded/system-closeable | CA prediction surface, Rule30/Rule90 decomposition, silent-boundary rules, Paper 06 Lucas support | infinite nonperiodicity/cold P3 promotion |}\\
...
\noindent\texttt{|---|---:|---|---|}\\
\noindent\texttt{| CQE-paper-12.25: Paper 12.25 - Toolkit for the CA Prediction Surface | 81 | Paper 12.25 describes the tools for reviewing the cellular-automaton prediction surface. These tools expose local readout, correction decompos}\\
...
\noindent\texttt{| CQE-paper-12.50: Paper 12.50 - CA Prediction Surface Claim Contract | 77 | Paper 12.50 lets later papers import CA prediction surfaces honestly. It keeps the exact local layer strong while preventing open prediction la}\\
\noindent\texttt{| Additional evidence cards | 22 total | The complete card inventory is maintained in the archive evidence matrix. | Cards are source-discovery aids until bound to paper-local evidence. |}\\
...
\noindent\texttt{|---|---|}\\
\noindent\texttt{| Publication ID | `FLCR-13` |}\\
\noindent\texttt{| Legacy source slot(s) | `12` |}\\
\noindent\texttt{| Ribbon role | order-2 slot-2: mark the correction, residue, vacancy, or mismatch surface |}\\
\noindent\texttt{| Proof form | residual accounting and bounded/unbounded split |}\\
\noindent\texttt{| Received state | state emitted by prior slot 11 and reopened at original slot 12 (CA Prediction Surface) |}\\
\noindent\texttt{| Produced state | formal result of original slot 12 plus the same-family lift path toward slot 22 |}\\

\subsubsection*{Claim-Evidence Table}

\noindent\texttt{| Claim | Lane | Evidence to attach | Boundary |}\\
\noindent\texttt{|---|---|---|---|}\\
\noindent\texttt{| Theorem 13.1 | `receipt\_bound\_internal\_result` | Receipt, source card, validator, citation, or CAM route named in the paper manifest | finite and bounded CA readouts can be receipt-closed without claiming unbounded Rul}\\
\noindent\texttt{| Proposition 13.2 | `normal\_form\_result` | Receipt, source card, validator, citation, or CAM route named in the paper manifest | CA prediction surface can be imported by later papers only through its declared source, re}\\
\noindent\texttt{| Protocol 13.3 | `falsifier\_or\_open\_obligation` | Receipt, source card, validator, citation, or CAM route named in the paper manifest | unbounded nonperiodicity, cold nth-bit extraction, and full prediction remain separ}\\

\subsubsection*{Figure Plan}

\noindent\texttt{| Figure | Purpose | Caption |}\\
\noindent\texttt{|---|---|---|}\\
\noindent\texttt{| FLCR-13-F1 | Residual split diagram | Schematic showing how `FLCR-13` turns its received state into the produced state while preserving claim lanes and residue boundaries. |}\\
\noindent\texttt{| FLCR-13-F2 | Evidence routing map | Diagram of source papers, archive cards, CAM/crystal routes, validators, and workbook replay paths used by this manuscript. |}\\
\noindent\texttt{| FLCR-13-F3 | Same-family lift context | Diagram placing this paper in the original `00-79` ribbon family and showing predecessor/successor slots. |}\\

\subsubsection*{In-Text Figure FLCR-13-F1: Residual split diagram}
...
\noindent\texttt{|---|---|}\\
\noindent\texttt{| FLCR-13-T1 | Residual accounting table |}\\
\noindent\texttt{| FLCR-13-T2 | Claim-lane/evidence-boundary table |}\\
\noindent\texttt{| FLCR-13-T3 | Archive evidence card and source-backed upgrade table |}\\
\noindent\texttt{| FLCR-13-T4 | Workbook replay and falsifier table |}\\
...
\subsubsection*{Appendix FLCR-13-A: Evidence Package}
...
`FLCR-13` (`Cellular Automata Prediction Surfaces`) occupies the `residual\_action` role at lift depth `1`.
...
\noindent\texttt{|---|---|---|}\\
\noindent\texttt{| Legacy sources | `12\_CA\_Prediction\_Surface.md`, `virtuous/12\_CA\_Prediction\_Surface\_VIRTUOUS.md`, `supplements/12\_INTERNAL\_REASONING\_SUPPLEMENT.md` | routed evidence |}\\
...
\noindent\texttt{|---|---|}\\
\noindent\texttt{| FLCR-13-F1: Residual split diagram | Visualizes the proof object, routing, or boundary. |}\\
\noindent\texttt{| FLCR-13-F2: Evidence routing map | Visualizes the proof object, routing, or boundary. |}\\
\noindent\texttt{| FLCR-13-F3: Same-family lift context | Visualizes the proof object, routing, or boundary. |}\\

\noindent\texttt{| Table | Role |}\\
\noindent\texttt{|---|---|}\\
\noindent\texttt{| FLCR-13-T1: Residual accounting table | Records claim lanes, evidence, sources, or falsifiers. |}\\
\noindent\texttt{| FLCR-13-T2: Claim-lane/evidence-boundary table | Records claim lanes, evidence, sources, or falsifiers. |}\\
\noindent\texttt{| FLCR-13-T3: Archive evidence card and source-backed upgrade table | Records claim lanes, evidence, sources, or falsifiers. |}\\
\noindent\texttt{| FLCR-13-T4: Workbook replay and falsifier table | Records claim lanes, evidence, sources, or falsifiers. |}\\
