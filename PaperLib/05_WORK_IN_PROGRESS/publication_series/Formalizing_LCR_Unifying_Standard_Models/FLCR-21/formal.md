...
An outside reviewer should read this paper as a formal-system construction paper. Its local object is **Materials Candidate Generation**. The paper's immediate contribution is: Keeps material candidate generation separate from fabrication performance claims.
...
**Core object.** Materials Candidate Generation.
...
`FLCR-21` should now be read as a resolved-state contribution for `Materials Candidate Generation` at its declared lane. The paper may state the strongest claim supported by these rows directly. It should reserve caution only for a specifically named stronger claim whose required evidence is absent.
