# FLCR-31 Workbook - Gauge Groups Translated Into LCR
...
- The `formal.md` file for `FLCR-31`.
...
| Step | Action |
|------|--------|
| Step 1 | Reconstruct the local object: gauge translation. |
| Step 2 | Write the receipt row that would demonstrate: gauge-like structures may be stated as translations only after the LCR-native carrier, atlas, face, and receipt dependencies are cited. |
| Step 3 | Route the residue without promoting it: measured gauge coupling values and physical identity require citation/calibration lanes. |
...
| additional routed sources | 13 more entries remain in `SOURCE_PLACEMENT.md` |
...
|---|---|---|---|
| Quark-face literalization | Open `quark_face_transport_literalized_receipt.json` and verify `passed=10`, `total=10`. | Three colors, S3 Weyl, exact n=3 closure, trace conservation, J3(O) partition all pass. | Physical quark identity and measured couplings remain calibration-bound. |
| Gauge target | Cite `SMD-02` and `SMD-03`. | External gauge vocabulary and `SU(3)_C x SU(2)_L x U(1)_Y` target are named. | Target definition is not an LCR identity proof. |
...
|---|---|---|
| Figures `FLCR-31-F1`, `FLCR-31-F2`, `FLCR-31-F3` | planned | Convert text schematics into final diagrams or leave as formal schematic figures. |
| Tables `FLCR-31-T1`-`FLCR-31-T4` | planned | Ensure all claim/evidence/source-card/workbook rows are complete. |
| Glossary | drafted | Replace generic term meanings with paper-local definitions. |
| Appendix FLCR-31-A | drafted | Attach receipt paths, verifier paths, source cards, and remaining external requirements. |
