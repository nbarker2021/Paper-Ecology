...
\usepackage{array}
\begin{document}
\section*{FLCR-18 - Exceptional Towers, VOA Routes, And Observer Face Selection}
...
\noindent{}-- Defines exceptional observer route as a first-class FLCR object.\\
\noindent{}-- States the local result: bounded exceptional, VOA, and observer-face readouts can be staged as receipt-bearing routes over the prior LCR carrier.\\
\noindent{}-- Routes downstream use through claim lanes rather than inherited prose: FLCR-38 and FLCR-40 may use these routes only after preserving bounded-vs-unbounded status.\\
\noindent{}-- Preserves the residue boundary: full Moonshine identification, measured observer physics, and unbounded exceptional closure remain separate obligations.\\
...
\noindent{}-- `17\_E6\_E8\_Error\_Correction\_Tower.md`\\
\noindent{}-- `18\_VOA\_Moonshine\_Representation\_Routes.md`\\
\noindent{}-- `19\_Observer\_Face\_Selection.md`\\
\noindent{}-- `supplements/17\_INTERNAL\_REASONING\_SUPPLEMENT.md`\\
\noindent{}-- `supplements/18\_INTERNAL\_REASONING\_SUPPLEMENT.md`\\
\noindent{}-- `supplements/19\_INTERNAL\_REASONING\_SUPPLEMENT.md`\\
...
\noindent{}-- **Exceptional route.** A staged path through E6/E8/VOA-style structures with bounded receipts.\\
\noindent{}-- **Observer face.** The selected chart/readout through which a latent state becomes active.\\
...
\noindent\texttt{|-------|------|-----------|}\\
\noindent\texttt{| Theorem 18.1 | `cam\_crystal\_reapplication\_result` | bounded exceptional, VOA, and observer-face readouts can be staged as receipt-bearing routes over the prior LCR carrier |}\\
\noindent\texttt{| Proposition 18.2 | `normal\_form\_result` | exceptional observer route can be imported by later papers only through its declared source, receipt, and lane. |}\\
\noindent\texttt{| Protocol 18.3 | `falsifier\_or\_open\_obligation` | full Moonshine identification, measured observer physics, and unbounded exceptional closure remain separate obligations |}\\
...
\noindent\texttt{|---|---|---|---|---|---|}\\
\noindent\texttt{| Theorem 18.1 | CAM route | bounded exceptional, VOA, and observer-face readouts can be staged as receipt-bearing routes over the prior LCR carrier | CAM/crystal route, source node, projection, window, or addressable ev}\\
\noindent\texttt{| Proposition 18.2 | formal construction | exceptional observer route can be imported by later papers only through its declared source, receipt, and lane | definitions, normal-form construction, conversion rule, and down}\\
\noindent\texttt{| Protocol 18.3 | obligation/falsifier | full Moonshine identification, measured observer physics, and unbounded exceptional closure remain separate obligations | explicit missing derivation, adapter, receipt, dataset, c}\\
...
\noindent\texttt{|---|---|---|---|}\\
\noindent\texttt{| Formal-system result | `FLCR-18` defines or uses **Exceptional Towers, VOA Routes, And Observer Face Selection** as a typed FLCR object with declared inputs, operations, outputs, and residue. | Definitions, formal clai}\\
...
\noindent\texttt{|---|---|---|---|}\\
\noindent\texttt{| paper lift enumeration | `receipt\_bound\_internal\_result` | 8/8 LCR states succeeded; error walls=0; avg TarPit mass=0.516120. | This closes the paper-local finite lift readout, not every stronger physical interpretatio}\\
...
\noindent\texttt{| exceptional/VOA observer machinery | `standard\_theorem\_citation\_bound\_result` | Exceptional-tower and VOA routing is closed where imported algebra and observer face selection are explicit. | Physical observer identity }\\
...
\noindent{}-- full Moonshine identification, measured observer physics, and unbounded exceptional closure remain separate obligations\\
...
Assigned original ribbon role(s): `17`/bridge\_action, `18`/terminal\_action, `19`/window\_action.

\noindent\texttt{| Original slot | Family | Lift | Role | Proof form |}\\
\noindent\texttt{|---|---|---:|---|---|}\\
\noindent\texttt{| `17` | bridge\_action | 1 | order-2 slot-7: project between discrete, continuous, lattice, or physical-facing forms | bridge, projection, calibration, or sample-preservation proof |}\\
\noindent\texttt{| `18` | terminal\_action | 1 | order-2 slot-8: land into lattice, game, runtime, or terminal address surfaces | terminal lookup, invariant split, and addressability proof |}\\
...
\noindent\texttt{|---|---|---:|---|}\\
\noindent\texttt{| `17` | bridge\_action | 1 | bridge, projection, calibration, or sample-preservation proof |}\\
\noindent\texttt{| `18` | terminal\_action | 1 | terminal lookup, invariant split, and addressability proof |}\\
...
\noindent\texttt{|---|---|}\\
\noindent\texttt{| `17\_E6\_E8\_Error\_Correction\_Tower.md` | primary assigned source for the paper's formal spine |}\\
\noindent\texttt{| `18\_VOA\_Moonshine\_Representation\_Routes.md` | primary assigned source for the paper's formal spine |}\\
\noindent\texttt{| `19\_Observer\_Face\_Selection.md` | primary assigned source for the paper's formal spine |}\\
\noindent\texttt{| `supplements/17\_INTERNAL\_REASONING\_SUPPLEMENT.md` | paper-local reasoning supplement contributing to definitions, proof sketch, and limitations |}\\
\noindent\texttt{| `supplements/18\_INTERNAL\_REASONING\_SUPPLEMENT.md` | paper-local reasoning supplement contributing to definitions, proof sketch, and limitations |}\\
...
\noindent\texttt{| `virtuous\textbackslash{}17\_E6-E8\_Error-Correction\_Tower\_VIRTUOUS.md` | prior enriched paper body; should be mined for mature claims and proof ordering |}\\
\noindent\texttt{| `virtuous\textbackslash{}18\_VOA\_\_\_Moonshine\_Representation\_Routes\_VIRTUOUS.md` | prior enriched paper body; should be mined for mature claims and proof ordering |}\\
\noindent\texttt{| `virtuous\textbackslash{}19\_Observer\_Face-Selection\_VIRTUOUS.md` | prior enriched paper body; should be mined for mature claims and proof ordering |}\\
...
\noindent\texttt{| 17 | 84 | bounded exceptional tower | E6/E8 tower, practical Leech lookup support from Paper 08, lattice forge surfaces | expanded Leech invariants, Weyl extractor, Gamma72 detail |}\\
\noindent\texttt{| 18 | 81 | bounded VOA/Moonshine route | centroid/VOA sector routes, Monster/McKay support from Papers 29/06/09 | full Moonshine identification and correction\_via\_voa route |}\\
...
\noindent\texttt{| Centroid / VOA sector chain | individual\_tool\_closure | `lattice\_forge/centroid\_voa.py` | Paper 18 | `centroid\_voa\_chain\_receipt.json` 5/5 | Finite VOA sector chain closes. |}\\
...
\noindent\texttt{|---|---:|---|---|}\\
\noindent\texttt{| CQE-paper-17.50: Paper 17.50 - Error-Correction Tower Claim Contract | 77 | Paper 17.50 keeps the tower strong by refusing to let its useful verified rungs hide the remaining obligations. | Source-backed support; promo}\\
\noindent\texttt{| CQE-paper-18.50: Paper 18.50 - VOA / Moonshine Route Claim Contract | 77 | Paper 18.50 keeps bounded route evidence useful without letting it become a hidden global proof claim. | Source-backed support; promotion requi}\\
...
\noindent\texttt{| SUMMARY-II-Folded-Strand-Physics: Summary Paper II — Folded Strand Physics: The Gluon as Quark, Mass, Curvature, Tower, and Moonshine | 75 | This paper presents the **physics applications** of the Gluon formalism. We d}\\
\noindent\texttt{| SUMMARY-II-Folded-Strand-Physics: Summary Paper II — Folded Strand Physics: The Gluon as Quark, Mass, Curvature, Tower, and Moonshine | 75 | This paper presents the **physics applications** of the Gluon formalism. We d}\\
\noindent\texttt{| Additional evidence cards | 36 total | The complete card inventory is maintained in the archive evidence matrix. | Cards are source-discovery aids until bound to paper-local evidence. |}\\
...
\noindent\texttt{|---|---|}\\
\noindent\texttt{| Publication ID | `FLCR-18` |}\\
\noindent\texttt{| Legacy source slot(s) | `17, 18, 19` |}\\
\noindent\texttt{| Ribbon role | order-2 slot-7: project between discrete, continuous, lattice, or physical-facing forms |}\\
\noindent\texttt{| Proof form | bridge, projection, calibration, or sample-preservation proof |}\\
\noindent\texttt{| Received state | state emitted by prior slot 16 and reopened at original slot 17 (E6 E8 Error Correction Tower) |}\\
\noindent\texttt{| Produced state | formal result of original slot 19 plus the same-family lift path toward slot 29 |}\\

\subsubsection*{Claim-Evidence Table}

\noindent\texttt{| Claim | Lane | Evidence to attach | Boundary |}\\
\noindent\texttt{|---|---|---|---|}\\
\noindent\texttt{| Theorem 18.1 | `cam\_crystal\_reapplication\_result` | Receipt, source card, validator, citation, or CAM route named in the paper manifest | bounded exceptional, VOA, and observer-face readouts can be staged as receipt-be}\\
\noindent\texttt{| Proposition 18.2 | `normal\_form\_result` | Receipt, source card, validator, citation, or CAM route named in the paper manifest | exceptional observer route can be imported by later papers only through its declared sourc}\\
\noindent\texttt{| Protocol 18.3 | `falsifier\_or\_open\_obligation` | Receipt, source card, validator, citation, or CAM route named in the paper manifest | full Moonshine identification, measured observer physics, and unbounded exceptional}\\

\subsubsection*{Figure Plan}

\noindent\texttt{| Figure | Purpose | Caption |}\\
\noindent\texttt{|---|---|---|}\\
\noindent\texttt{| FLCR-18-F1 | Correspondence bridge | Schematic showing how `FLCR-18` turns its received state into the produced state while preserving claim lanes and residue boundaries. |}\\
\noindent\texttt{| FLCR-18-F2 | Evidence routing map | Diagram of source papers, archive cards, CAM/crystal routes, validators, and workbook replay paths used by this manuscript. |}\\
\noindent\texttt{| FLCR-18-F3 | Same-family lift context | Diagram placing this paper in the original `00-79` ribbon family and showing predecessor/successor slots. |}\\

\subsubsection*{In-Text Figure FLCR-18-F1: Correspondence bridge}
...
\noindent\texttt{|---|---|}\\
\noindent\texttt{| FLCR-18-T1 | Source-target correspondence table |}\\
\noindent\texttt{| FLCR-18-T2 | Claim-lane/evidence-boundary table |}\\
\noindent\texttt{| FLCR-18-T3 | Archive evidence card and source-backed upgrade table |}\\
\noindent\texttt{| FLCR-18-T4 | Workbook replay and falsifier table |}\\
...
\subsubsection*{Appendix FLCR-18-A: Evidence Package}
...
`FLCR-18` (`Exceptional Towers, VOA Routes, And Observer Face Selection`) occupies the `bridge\_action` role at lift depth `1`.
...
\noindent\texttt{|---|---|---|}\\
\noindent\texttt{| Legacy sources | `17\_E6\_E8\_Error\_Correction\_Tower.md`, `18\_VOA\_Moonshine\_Representation\_Routes.md`, `19\_Observer\_Face\_Selection.md`, `supplements/17\_INTERNAL\_REASONING\_SUPPLEMENT.md`, `supplements/18\_INTERNAL\_REASONING}\\
...
\noindent\texttt{|---|---|}\\
\noindent\texttt{| FLCR-18-F1: Correspondence bridge | Visualizes the proof object, routing, or boundary. |}\\
\noindent\texttt{| FLCR-18-F2: Evidence routing map | Visualizes the proof object, routing, or boundary. |}\\
\noindent\texttt{| FLCR-18-F3: Same-family lift context | Visualizes the proof object, routing, or boundary. |}\\

\noindent\texttt{| Table | Role |}\\
\noindent\texttt{|---|---|}\\
\noindent\texttt{| FLCR-18-T1: Source-target correspondence table | Records claim lanes, evidence, sources, or falsifiers. |}\\
\noindent\texttt{| FLCR-18-T2: Claim-lane/evidence-boundary table | Records claim lanes, evidence, sources, or falsifiers. |}\\
\noindent\texttt{| FLCR-18-T3: Archive evidence card and source-backed upgrade table | Records claim lanes, evidence, sources, or falsifiers. |}\\
\noindent\texttt{| FLCR-18-T4: Workbook replay and falsifier table | Records claim lanes, evidence, sources, or falsifiers. |}\\
