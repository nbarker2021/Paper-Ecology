...
- Dimensional role: order-1 slot-1: start the active one-dimensional action / enumerate the center state
...
| Archive source artifacts | 169 | Supply source bundles and extracted candidate files requiring source-card conversion. |
| Local unintegrated artifacts | 116 | Discovery backlog for exact integration and external pairing. |
...
|---|---|---|
| Assembly audit | {'ASSEMBLE': 11, 'REROUTE_OR_REPAIR': 80, 'DISCOVERY_EXTERNAL_PAIR_REQUIRED': 208, 'DISCOVERY_MORE_CONTEXT_REQUIRED': 6} | Only `ASSEMBLE` records support result claims. |
...
**Claim 01.1.** The validated pair `PEEP-2026-006` may be used as a bounded comparator for Lattice Forge Inventory As Idempotent-Semiring Basis Comparator. The claim is limited to the correlation and validation lane recorded in the evidence notebook.

**Claim 01.2.** The validated pair `PEEP-2026-012` may be used as a bounded comparator for D12 Idempotent Chain Tests As Semicentral-Idempotent Comparator. The claim is limited to the correlation and validation lane recorded in the evidence notebook.

**Claim 01.3.** The validated pair `PEEP-2026-013` may be used as a bounded comparator for D4 J30 Idempotent Bridge Tests As Finite-Basis Semiring Comparator. The claim is limited to the correlation and validation lane recorded in the evidence notebook.

**Claim 01.4.** The validated pair `PEEP-2026-014` may be used as a bounded comparator for D12 Action Verifier As Semicentral-Idempotent Action Comparator. The claim is limited to the correlation and validation lane recorded in the evidence notebook.
...
|---|---|---|
| 01 | 26c24e0b2ed810ca | D:\CQE_CMPLX\papers\active-rework\publication_series\Formalizing_LCR_Unifying_Standard_Models\CAM_CRYSTAL_MEMORY_BANK\crystals\slot_crystal\01.26c24e0b2ed8.json |

## Appendix C: CAM Database Provenance

| Database | Port recommendation | Signal tables | Use |
|---|---|---|---|
| D:\CQE_CMPLX\TMN_TOOLS_LCR.db | REVIEW_FOR_PORT | lcr_tiles, lcr_tools, sql_crystals, tool_atoms | Create exact table/row source cards; use for CAM/provenance/receipt support. |
| D:\CQE_CMPLX\git-hosted-roots\CQECMPLX-Production\archive\stale-review\2026-06-27\produ... | REVIEW_FOR_PORT | lcr_tiles, lcr_tools, sql_crystals, tool_atoms | Create exact table/row source cards; use for CAM/provenance/receipt support. |
