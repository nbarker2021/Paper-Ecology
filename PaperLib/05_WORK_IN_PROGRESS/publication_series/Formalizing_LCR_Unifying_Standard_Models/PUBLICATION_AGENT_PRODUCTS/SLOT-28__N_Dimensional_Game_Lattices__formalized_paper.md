...
This paper product formalizes slot `28` as a terminal lookup, address closure, invariant split, and stable library object. Within the 1-80 publication ribbon, this slot uses the method role 'make a reusable lookup or invariant object' and the physics-facing role 'define lookup-ready constants, stable phases, invariant classes, or terminal observables'. The current assembly pass attaches 280 evidence records, including 32 CAM database candidates, 130 archive-source candidates, 2 validated pairwise packages, and 3 additional external publication candidates. Claims are restricted to records that pass the assembly audit; all other material is retained as obligations or source-card queues.

## Keywords

observer-state formalism, 28, terminal_action, CAM, crystal memory, receipt-bound evidence, validation

## 1. Purpose And Slot Definition

Slot `28` is treated as a proof-action address, not as a loose topic bucket. Its generalized action is: **terminal lookup, address closure, invariant split, and stable library object**.
...
- Dimensional role: order-3 slot-8: land into lattice, game, runtime, or terminal address surfaces
...
|---|---|---|
| Validated pairwise packages | 2 | May support result claims if assembly-ready. |
| CAM database port candidates | 32 | Support provenance, memory, receipts, source routing, and row-level source-card creation. |
| Archive source artifacts | 130 | Supply source bundles and extracted candidate files requiring source-card conversion. |
| Local unintegrated artifacts | 114 | Discovery backlog for exact integration and external pairing. |
...
|---|---|---|
| Assembly audit | {'ASSEMBLE': 2, 'REROUTE_OR_REPAIR': 83, 'DISCOVERY_EXTERNAL_PAIR_REQUIRED': 190, 'DISCOVERY_MORE_CONTEXT_REQUIRED': 5} | Only `ASSEMBLE` records support result claims. |
...
**Claim 28.1.** The validated pair `PEEP-2026-014` may be used as a bounded comparator for D12 Action Verifier As Semicentral-Idempotent Action Comparator. The claim is limited to the correlation and validation lane recorded in the evidence notebook.

**Claim 28.2.** The validated pair `PEEP-2026-015` may be used as a bounded comparator for D12 Moonshine Chain As Umbral Trace-Function Comparator. The claim is limited to the correlation and validation lane recorded in the evidence notebook.
...
|---|---|---|
| 28 | c4e1c0b538f80f1a | D:\CQE_CMPLX\papers\active-rework\publication_series\Formalizing_LCR_Unifying_Standard_Models\CAM_CRYSTAL_MEMORY_BANK\crystals\slot_crystal\28.c4e1c0b538f8.json |
...
| D:\CQE_CMPLX\kernel\staging\papers\suite83_cmplx_kernel_production\CQE_CMPLX-workspace\... | zip | 63 | 63 | Review staged extraction and convert relevant files into exact source cards. |
| D:\CQE_CMPLX\kernel\staging\papers\suite81_claude_codex_memory\Repo zips\CMPLXUNI-main.zip | zip | 636 | 605 | Review staged extraction and convert relevant files into exact source cards. |
| D:\CQE_CMPLX\_memory\claude-codex\Repo zips\CMPLXUNI-main.zip | zip | 636 | 605 | Review staged extraction and convert relevant files into exact source cards. |
| D:\CQE_CMPLX\g\CMPLX-1T\Wolfram study\CMPLX-Alpha-Housing.tar.gz | tar | 147 | 142 | Review staged extraction and convert relevant files into exact source cards. |
| D:\CQE_CMPLX\kernel\staging\papers\suite81_claude_codex_memory\Repo zips\CMPLXDevKit-ma... | zip | 662 | 643 | Review staged extraction and convert relevant files into exact source cards. |
