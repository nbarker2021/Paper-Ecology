...
```text
received state -> QCD And Color Transport In LCR -> produced state
```
...
This paper now has a concrete QCD-side comparator: the quark-face transport receipts close a finite color-transport surface and the SMD layer defines the external QCD target. That changes the paper from “QCD analogy pending” to “structural color-transport row closed, physical QCD dynamics bounded.”

Reader translation: the closed row is the algebraic color/face transport. The still-bounded rows are confinement dynamics, running coupling, hadron spectra, CKM/weak mixing, and any claim that the finite surface alone derives measured QCD.
...
For `FLCR-32`, the most important figure is `FLCR-32-F1`: Representation fold atlas.
The most important table is `FLCR-32-T1`: Face/representation correspondence table.
