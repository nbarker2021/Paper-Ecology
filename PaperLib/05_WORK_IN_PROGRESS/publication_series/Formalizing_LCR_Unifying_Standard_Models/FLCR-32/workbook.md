# FLCR-32 Workbook - QCD And Color Transport In LCR
...
- The `formal.md` file for `FLCR-32`.
...
| Step | Action |
|------|--------|
| Step 1 | Reconstruct the local object: QCD color translation. |
| Step 2 | Write the receipt row that would demonstrate: finite six-face transport can support a QCD-facing translation only with explicit scope separating structural color from measured QCD. |
| Step 3 | Route the residue without promoting it: physical quark confinement, CKM, and measured QCD parameters remain external calibration. |
...
Analog invariance test: a folded map: the visible geometry changes while the indexed locations remain traceable.
...
| additional routed sources | 12 more entries remain in `SOURCE_PLACEMENT.md` |
...
| Ckm Sm Transport Split | Separate algebraic transport receipt from CKM/PDG value table and calibration claim. | Physical-quark identity, measured parameters, and calibration remain citation/data-bound. |
...
|---|---|---|---|
| Color transport receipt | Open `quark_face_transport_receipt.json`; inspect shell-2 states, trace-2 idempotents, S3 closure, and exact SU(3) residual. | S3 closes on the trace-2 triple and residual squared is zero. | Full QCD dynamics, confinement, and spectra remain external comparator rows. |
| QCD target | Cite `SMD-05` and `SMD-08`. | External QCD/running-parameter objects are named before translation. | Does not close measured QCD phenomenology. |
...
|---|---|---|
| Figures `FLCR-32-F1`, `FLCR-32-F2`, `FLCR-32-F3` | planned | Convert text schematics into final diagrams or leave as formal schematic figures. |
| Tables `FLCR-32-T1`-`FLCR-32-T4` | planned | Ensure all claim/evidence/source-card/workbook rows are complete. |
| Glossary | drafted | Replace generic term meanings with paper-local definitions. |
| Appendix FLCR-32-A | drafted | Attach receipt paths, verifier paths, source cards, and remaining external requirements. |
