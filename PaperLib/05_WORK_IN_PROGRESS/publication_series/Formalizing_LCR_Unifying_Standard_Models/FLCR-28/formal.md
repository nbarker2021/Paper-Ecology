# FLCR-28 - CAM, Crystal Projectors, And Runtime
...
- Routes downstream use through claim lanes rather than inherited prose: this paper becomes the runtime intake lane for agent-generated, standards-window, runtime, notebook-derived, and future crystal work.
...
|---|---|---|---|---|---|
| Theorem 28.1 | CAM route | CAM-addressed objects can be projected through multiple representational faces while remaining addressable by the same memory contract | CAM/crystal route, source node, projection, window, or addressable evidence artifact | routing and reapplication evidence; not a standalone physical proof | check the CAM source, route, and attached data are named |
| Proposition 28.2 | formal construction | crystal projector can be imported by later papers only through its declared source, receipt, and lane | definitions, normal-form construction, conversion rule, and downstream-use boundary | internal formal coherence; no measured physical identity without a separate calibration row | check that the normal form is named and the conversion rule is explicit |
| Protocol 28.3 | obligation/falsifier | private or missing CAM vault data must remain marked as pending rather than assumed | explicit missing derivation, adapter, receipt, dataset, comparator, or failed test condition | does not negate satisfied lower-level rows | name the next binding action and owner surface |
...
|---|---|---|---|
| Formal-system result | `FLCR-28` defines or uses **CAM, Crystal Projectors, And Runtime** as a typed FLCR object with declared inputs, operations, outputs, and residue. | Definitions, formal claims, construction steps, and downstream-use rules are internally consistent and lane typed. | External physical truth, measured prediction, or identity with a standard theory. |
...
|---|---|
| `local evidence artifact` | Defines CAM-first routing and the wall/gating discipline this paper imports. |
| `local evidence artifact` | Makes runtime errors correction lanes rather than terminal failures. |
| `local evidence artifact` | Allows adapter construction by function and receipt rather than by stale names. |
| `local evidence artifact` | Current MCP surface; evidence for adapter existence and for the remaining first-routing obligation. |
...
|---|---|---|---|
| paper lift enumeration | `receipt_bound_internal_result` | 8/8 LCR states succeeded; error walls=0; avg TarPit mass=0.510159. | This closes the paper-local finite lift readout, not every stronger physical interpretation. |
...
| CAM/crystal projector | `cam_crystal_reapplication_result` | The crystal projector is closed as an address-preserving multi-face projection protocol. | Private vault contents remain unavailable until attached. |
...
`FLCR-28` (`CAM, Crystal Projectors, And Runtime`) occupies the `publication_overlay` role at lift depth `n/a`.
...
|---|---|---|
| Legacy sources | `supplements/CRYSTAL_CAM_PROJECTOR_SUPPLEMENT.md`, `AGENT_CRYSTAL_WORK_HARVEST.md`, `PAPER_REWORKS_CRYSTAL_REPORT.md` | routed evidence |
...
| Figure | Role |
|---|---|
| FLCR-28-F1: Publication overlay dependency map | Visualizes the proof object, routing, or boundary. |
| FLCR-28-F2: Evidence routing map | Visualizes the proof object, routing, or boundary. |
| FLCR-28-F3: Same-family lift context | Visualizes the proof object, routing, or boundary. |

| Table | Role |
|---|---|
| FLCR-28-T1: Dependency and translation table | Records claim lanes, evidence, sources, or falsifiers. |
| FLCR-28-T2: Claim-lane/evidence-boundary table | Records claim lanes, evidence, sources, or falsifiers. |
| FLCR-28-T3: Archive evidence card and source-backed upgrade table | Records claim lanes, evidence, sources, or falsifiers. |
| FLCR-28-T4: Workbook replay and falsifier table | Records claim lanes, evidence, sources, or falsifiers. |
