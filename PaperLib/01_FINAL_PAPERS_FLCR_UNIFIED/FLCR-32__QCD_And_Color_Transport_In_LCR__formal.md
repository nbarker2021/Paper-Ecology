...
An outside reviewer should read this paper as a translation and comparator paper. Its local object is **QCD And Color Transport In LCR**. The paper's immediate contribution is: Maps QCD/color transport after algebraic face transport is established.
...
**Core object.** QCD And Color Transport In LCR.
...
`FLCR-32` should now be read as a resolved-state contribution for `QCD And Color Transport In LCR` at its declared lane. The paper may state the strongest claim supported by these rows directly. It should reserve caution only for a specifically named stronger claim whose required evidence is absent.
