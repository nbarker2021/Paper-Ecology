# SMD-08 Workbook - ﻿# Renormalization, Effective Field Theory, And Running Parameters

## Purpose

This workbook turns `SMD-08` into a concrete Standard Model definition and comparator exercise. It is not a blank checklist. It is the place where a reviewer can take a
Standard Model-facing claim and decide exactly what closes now, what is merely
defined by external formalism, what is closed by FLCR proof, what is
calibration-bound, and what remains a falsifier/open obligation.

## Formal Paper Digest

This paper defines **scale and renormalization sector** as an external Standard Model or standard-physics object before any LCR translation is attempted. It binds accepted formalism, IRL sources, FLCR claim lanes, and falsifier boundaries. The strongest current claim is: Any claim that changes by scale must carry its scale, scheme, matching rule, and allowed error or falsifier. The paper also identifies what can be treated as closed now and what remains data-bound, calibration-bound, or unresolved.

## Claim Routing Table

| Claim | Lane | Statement | Workbook action |
|---|---|---|---|
| Theorem 08.1 | `standard_theorem_citation_bound_result` | Any claim that changes by scale must carry its scale, scheme, matching rule, and allowed error or falsifier. | Attach source/receipt/comparator and mark closure state. |
| Proposition 08.2 | `normal_form_result` | This definition can be consumed by an FLCR paper only after the external object, LCR mapping, boundary, and destination paper are named. | Attach source/receipt/comparator and mark closure state. |

## IRL Source Rows To Use

| Source | Description | Workbook treatment |
|---|---|---|
| `IRL-PDG-2026` | Particle Data Group Review of Particle Physics 2026 | Use as external definition or calibration source. |
| `IRL-PDG-2024` | Review of Particle Physics 2024/2025, Phys. Rev. D 110, 030001 | Use as external definition or calibration source. |
| `IRL-CODATA-2022` | NIST CODATA Recommended Values of the Fundamental Physical Constants: 2022 | Use as external definition or calibration source. |

## Closure-Now Rows

| Row | Closure statement | How to use it |
|---|---|---|
| Closure row 1 | Close scale-dependence as a definition when scale, scheme, and domain are stated. | Cite in destination paper only at stated scope. |
| Closure row 2 | Close zero-cost lookup only as an internal retrieval claim, not as a substitute for renormalization context. | Cite in destination paper only at stated scope. |

## Open Or Calibration-Bound Rows

| Row | Obligation | Required next evidence |
|---|---|---|
| Obligation 1 | Running-coupling agreement remains open without scale/scheme/value/uncertainty. | Route to calibration, comparator, or falsifier. |
| Obligation 2 | Cross-scale synthesis claims need matching rules. | Route to calibration, comparator, or falsifier. |

## Destination FLCR Routing

| Destination | Required use |
|---|---|
| FLCR-01 | Cite `SMD-08` before using this Standard Model-facing object; attach lane, comparator, and boundary. |
| FLCR-31 | Cite `SMD-08` before using this Standard Model-facing object; attach lane, comparator, and boundary. |
| FLCR-32 | Cite `SMD-08` before using this Standard Model-facing object; attach lane, comparator, and boundary. |
| FLCR-33 | Cite `SMD-08` before using this Standard Model-facing object; attach lane, comparator, and boundary. |
| FLCR-37 | Cite `SMD-08` before using this Standard Model-facing object; attach lane, comparator, and boundary. |
| FLCR-39 | Cite `SMD-08` before using this Standard Model-facing object; attach lane, comparator, and boundary. |
| FLCR-40 | Cite `SMD-08` before using this Standard Model-facing object; attach lane, comparator, and boundary. |
| FLCR-02 | Cite `SMD-08` before using this Standard Model-facing object; attach lane, comparator, and boundary. |
| FLCR-18 | Cite `SMD-08` before using this Standard Model-facing object; attach lane, comparator, and boundary. |
| FLCR-19 | Cite `SMD-08` before using this Standard Model-facing object; attach lane, comparator, and boundary. |
| FLCR-30 | Cite `SMD-08` before using this Standard Model-facing object; attach lane, comparator, and boundary. |

## Comparator Exercise

For each destination FLCR claim, fill the row below. A row is allowed to close only
at the scope supported by its evidence lane.

| Field | Required value |
|---|---|
| Claim | Copy the exact sentence from the FLCR or SMD paper. |
| External object | Name the Standard Model or standard-physics object defined by `SMD-08`. |
| External evidence | Cite one IRL source row or mark as missing. |
| FLCR object | Name the carrier, face, residue, bridge, ledger, CAM/crystal route, normal form, or receipt being mapped. |
| Lane | Choose receipt-bound, citation-bound, CAM/crystal, normal-form, calibration, synthesis dependency, or falsifier/open obligation. |
| Comparator | State what would count as match, mismatch, partial correspondence, or out-of-scope. |
| Boundary | State what the row does not prove. |
| Falsifier | State what observation, failed derivation, or missing dependency blocks promotion. |
| Closure state | closed by standard formalism / closed by FLCR proof / CAM-crystal reapplied / calibration-bound / open obligation |

## Worked Closure Pattern

```text
claim -> SMD-08 definition -> destination FLCR dependency -> lane
      -> evidence object -> boundary -> falsifier -> closure state
```

Use the pattern aggressively: the goal is not to preserve uncertainty by habit,
but to close every row whose evidence object is already present and to name the
exact missing object for every row that remains open.

## CAM/Crystal Query Patterns

```text
SMD-08 source registry
SMD-08 destination FLCR-01 FLCR-31 FLCR-32 FLCR-33 FLCR-37 FLCR-39 FLCR-40 FLCR-02 FLCR-18 FLCR-19 FLCR-30
SMD-08 closure rows
SMD-08 comparator falsifier boundary
SMD-08 CAM crystal reapplication evidence
```

## Analog Review Route

Use two ledgers side by side. The left ledger names the external Standard Model
object and citation. The right ledger names the LCR/FLCR object. A string or
token connects them only when the comparator row has a lane, evidence object,
boundary, and falsifier. If the string cannot be attached, the claim is not
discarded; it becomes an open channel with a named missing object.

## Completion Criteria

- Every claim has a lane and closure state.
- Every external object has a source row.
- Every FLCR mapping names the exact destination paper.
- Every calibration-bound claim has units, dataset, uncertainty, and comparator
  requirements named.
- Every large synthesis statement names its dependencies rather than relying on
  rhetorical force.
