# FLCR-19 Workbook - Layer-2 Synthesis Ledger
...
- The `formal.md` file for `FLCR-19`.
...
| Step | Action |
|------|--------|
| Step 1 | Reconstruct the local object: layer-2 synthesis ledger. |
| Step 2 | Write the receipt row that would demonstrate: prior receipt-bearing claims can be assembled into a monotone ledger when edge type and claim lane are preserved. |
| Step 3 | Route the residue without promoting it: unknown and forbidden reachability must remain explicit rather than hidden in summary prose. |
...
Analog invariance test: a sealed ledger or completed window whose contents are preserved while the next layer receives only typed imports.
...
|---|---|---|
| Formal Methods Receipt Closure | Attach envelope, receipt, lane grant, replay path, and dependency row for each governed claim. | Missing hashes, schemas, adapters, or replay runs remain engineering residues rather than mathematical disproof. |
| Negative Receipts As Guards | Record rejected candidate, failure mode, blocked promotion, and remaining positive-proof requirement. | A negative receipt blocks a promotion; it does not prove a positive replacement unless separately evidenced. |
...
|---|---|---|
| Figures `FLCR-19-F1`, `FLCR-19-F2`, `FLCR-19-F3` | planned | Convert text schematics into final diagrams or leave as formal schematic figures. |
| Tables `FLCR-19-T1`-`FLCR-19-T4` | planned | Ensure all claim/evidence/source-card/workbook rows are complete. |
| Glossary | drafted | Replace generic term meanings with paper-local definitions. |
| Appendix FLCR-19-A | drafted | Attach receipt paths, verifier paths, source cards, and remaining external requirements. |
