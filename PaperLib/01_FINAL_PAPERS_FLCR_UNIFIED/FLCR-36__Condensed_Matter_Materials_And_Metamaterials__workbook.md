# FLCR-36 Workbook - Condensed Matter, Materials, And Metamaterials
...
- The `formal.md` file for `FLCR-36`.
...
| Step | Action |
|------|--------|
| Step 1 | Reconstruct the local object: materials translation. |
| Step 2 | Write the receipt row that would demonstrate: forge-generated material candidates can be translated into condensed-matter hypotheses when descriptors, band context, and validation lane are attached. |
| Step 3 | Route the residue without promoting it: fabrication, finite-element performance, spectroscopy, and device claims remain empirical. |
...
Analog invariance test: a correction ledger: the label on the defect can change, but the unresolved difference remains the same object.
...
| additional routed sources | 20 more entries remain in `SOURCE_PLACEMENT.md` |
...
|---|---|---|
| Symbolic Vs Physical Carrier | List every carrier phrase and classify it as symbolic address, standard-physics correspondence, or calibrated physical claim. | Physical identity, units, Hamiltonians, datasets, and measured constants remain external-calibration claims. |
| Electron Hole Exciton Accounting | Build a correspondence table: LCR residue term, standard condensed-matter term, equation/citation slot, and remaining calibration need. | Quantitative material behavior requires model parameters, units, citations, and data. |
| Applied Forge Internal External Split | Record input object, forge operation, output descriptor/candidate, validator result, and external-validation boundary. | Fabrication, wet-lab, biological, gameplay-global, manufacturing, or measured optimization claims remain external-validation needs. |
...
## Executable Evidence Replay

| Replay target | Concrete action | Pass condition | Boundary |
|---|---|---|---|
| MetaForge materials receipt | Open `metaforge_materials_receipt.json`; verify `status=pass_with_open_obligations`. | Candidate ledger includes inventory, Pareto partner, ten-fold evaluation, seams, production accounting, and replay pairs. | Measured/fabricated material performance remains open. |
| Graphene/hBN row | Inspect top partner and Pareto score. | hBN selected as top partner for Graphene with score 0.89. | Candidate estimate is not a measured material property. |

## Online Source Replay

| Replay target | Action | Pass condition | Boundary |
|---|---|---|---|
| Online source row | Open ONLINE_SOURCE_APPLICATION_PASS.json and find this paper's paper_routes entry. | Every online source used by this paper is listed with source ID, URL, lane, and closure effect. | Source availability does not equal LCR identity proof. |
| Registry sync | Check IRL_STANDARD_MODEL_SOURCE_REGISTRY.json for the same source IDs. | IDs match and lane assignments are present. | Any stronger claim still needs receipt/comparator/calibration. |

## Assertive Closure Replay

| Replay target | Action | Pass condition | Boundary |
|---|---|---|---|
| Assertive closure row | Open ASSERTIVE_CLOSURE_APPLICATION_PASS.json and find this paper's entry. | The paper's closure promotion is stated as a lane-specific result. | The stronger claim remains residue unless its own lane is satisfied. |
| Residue split | For one formerly open row, split closed now from still not promoted. | The row is no longer generically open. | No unconditional physical identity claim is introduced. |
...
|---|---|---|
| Figures `FLCR-36-F1`, `FLCR-36-F2`, `FLCR-36-F3` | planned | Convert text schematics into final diagrams or leave as formal schematic figures. |
| Tables `FLCR-36-T1`-`FLCR-36-T4` | planned | Ensure all claim/evidence/source-card/workbook rows are complete. |
| Glossary | drafted | Replace generic term meanings with paper-local definitions. |
| Appendix FLCR-36-A | drafted | Attach receipt paths, verifier paths, source cards, and remaining external requirements. |
