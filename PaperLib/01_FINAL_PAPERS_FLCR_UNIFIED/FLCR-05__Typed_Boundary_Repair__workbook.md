# FLCR-05 Workbook - Typed Boundary Repair
...
- The `formal.md` file for `FLCR-05`.
...
| Step | Action |
|------|--------|
| Step 1 | Transform a raw failure message into a valid repair row. |
| Step 2 | Run the repair transform twice and confirm the second pass is unchanged. |
| Step 3 | Route the row to one allowed next paper and one rejected next paper, explaining the boundary. |
...
Analog invariance test: a membrane or interface: crossing rules change access, not the identity of the carried state.
...
|---|---|---|
| Figures `FLCR-05-F1`, `FLCR-05-F2`, `FLCR-05-F3` | planned | Convert text schematics into final diagrams or leave as formal schematic figures. |
| Tables `FLCR-05-T1`-`FLCR-05-T4` | planned | Ensure all claim/evidence/source-card/workbook rows are complete. |
| Glossary | drafted | Replace generic term meanings with paper-local definitions. |
| Appendix FLCR-05-A | drafted | Attach receipt paths, verifier paths, source cards, and remaining external requirements. |
