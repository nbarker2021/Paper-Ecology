...
|---|---|---|---|---|---|
| Theorem 34.1 | external target or comparator | standard electron, hole, recombination, screening, and exciton models can explain part of the open residue surface when material context is declared | named external theorem, definition layer, citation target, or imported standard formalism | imports the external object as-is; does not silently reprove or redefine it | attach final citation metadata and mapping rule |
| Proposition 34.2 | translation grammar | exciton accounting can be imported by later papers only through its declared source, receipt, and lane | definitions, normal-form construction, conversion rule, and downstream-use boundary | internal formal coherence; no measured physical identity without a separate calibration row | check that the normal form is named and the conversion rule is explicit |
| Protocol 34.3 | obligation/falsifier | material-specific bands, lifetimes, mobilities, and spectra require data-bound calibration | explicit missing derivation, adapter, receipt, dataset, comparator, or failed test condition | does not negate satisfied lower-level rows | name the next binding action and owner surface |
...
|---|---|---|---|
| Formal-system result | `FLCR-34` defines or uses **Electron-Hole-Exciton Accounting** as a typed FLCR object with declared inputs, operations, outputs, and residue. | Definitions, formal claims, construction steps, and downstream-use rules are internally consistent and lane typed. | External physical truth, measured prediction, or identity with a standard theory. |
...
| Exciton source registry | `local evidence artifact` | Supplies `IRL-EXCITON-SPRINGER` and `IRL-EXCITON-2D-REVIEW` as citation-bound external sources. | External sources close definitions and formal targets, not FLCR-specific physical identity. |
| NSIT reasoned closure pass | `local evidence artifact` | States that electron-hole-exciton formalism closes a large part of the vacancy/complement vocabulary for papers 07, 15, 16, and 22. | Requires material model, Hamiltonian/equations, units, and citation/data before measured claims promote. |
...
|---|---|---|---|
| paper lift enumeration | `receipt_bound_internal_result` | 8/8 LCR states succeeded; error walls=0; avg TarPit mass=0.513090. | This closes the paper-local finite lift readout, not every stronger physical interpretation. |
...
| electron-hole-exciton formalism | `standard_theorem_citation_bound_result` | Electron-hole-exciton vocabulary is closed by standard condensed-matter formalism. | Material-specific exciton values require material parameters. |
...
`FLCR-34` (`Electron-Hole-Exciton Accounting`) occupies the `publication_overlay` role at lift depth `n/a`.
...
|---|---|---|
| Legacy sources | `NP-12_Electron_Hole_Exciton_Accounting_For_Open_Math.md`, `supplements/SM_BRIDGE_SUPPLEMENT.md` | routed evidence |
...
| Figure | Role |
|---|---|
| FLCR-34-F1: Publication overlay dependency map | Visualizes the proof object, routing, or boundary. |
| FLCR-34-F2: Evidence routing map | Visualizes the proof object, routing, or boundary. |
| FLCR-34-F3: Same-family lift context | Visualizes the proof object, routing, or boundary. |

| Table | Role |
|---|---|
| FLCR-34-T1: Dependency and translation table | Records claim lanes, evidence, sources, or falsifiers. |
| FLCR-34-T2: Claim-lane/evidence-boundary table | Records claim lanes, evidence, sources, or falsifiers. |
| FLCR-34-T3: Archive evidence card and source-backed upgrade table | Records claim lanes, evidence, sources, or falsifiers. |
| FLCR-34-T4: Workbook replay and falsifier table | Records claim lanes, evidence, sources, or falsifiers. |
